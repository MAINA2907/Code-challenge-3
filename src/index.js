...
document.addEventListener('DOMContentLoaded', () => {
    movieDetails(); // Fetching movie details and displaying them
    remainingTickets(); // Displaying remaining tickets for each movie
});

// Function to display and fetch movie details
function movieDetails() {
    const ul = document.getElementById("films");
    fetch("https://json-server-6lbf.onrender.com/films")
...
function buyTicket(movieId) {
    fetch(`https://json-server-6lbf.onrender.com/films/${movieId}`)
    .then(response => response.json())
    .then(movie => {
        if (movie.tickets_sold < movie.capacity) {
...
// Function displaying remaining tickets for each movie
